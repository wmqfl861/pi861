import { PostgresStateStore } from "../src/live/store.ts";
	...
	try {
		...
		await t.test("versions, invalidation events and receipts exist in matching counts", async () => {
			...
			try { await connection.query(`SET search_path TO "${schema}"`);
				for (const table of ["pi861_memory_versions", "pi861_memory_outbox", "pi861_memory_receipts"]) assert.equal(Number((await connection.query(`SELECT count(*) AS n FROM ${table}`)).rows[0].n), 3);
